...
import { getBackendUrl, getBackendWsUrl } from "../network/host.js";
...
function initGlobalSocket() {
    ...
    globalSocket = new WebSocket(getBackendWsUrl('/ws'));
...
function loadDeviceSet(robotRecord, opcUaUrl) {
    ...
    fetch(getBackendUrl(`/device_set_rendered?url=${encodedUrl}`))
        .then(res => res.text())
        .then(html => {
...
export function connectOpcUa(robotRecord) {
    ...
    const socket = connectivity.socket || initGlobalSocket();
    if (socket && socket.readyState === WebSocket.OPEN) {
        connectivity.socket = socket;
        socket.send(message);
