...
import { getBackendUrl } from '../network/host.js';
...
export function handleOpcUaNodeSelection(robotRecord, event) {
    ...
    fetch(getBackendUrl(`/references?url=${encodedUrl}&nodeid=${encodedNodeId}`))
        .then(res => res.json())
        .then(refs => {
...
export async function handleSubtreeClick(robotRecord, e) {
    ...
    if (details && !details.open && !ul.classList.contains("subtree-loaded")) {
        ...
        const resp = await fetch(getBackendUrl(`/subtree_children?url=${encodedUrl}&nodeid=${nodeId}`));
...
export function refreshSelectedNode(robotRecord) {
    ...
    if (nodeClass == "2") {
        ...
        const encodedUrl = encodeURIComponent(connectivity.connectedUrl);
        const encodedNodeId = encodeURIComponent(ui.selectedNodeId);
        fetch(getBackendUrl(`/node_rendered?url=${encodedUrl}&nodeid=${encodedNodeId}&children_depth=1`))
            .then(res => res.text())
            .then(html => {
                const staging = document.createElement('div');
                staging.innerHTML = html;
