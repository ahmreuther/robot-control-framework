...
- Live data from the physical robot is transferred (activate sync for Joint streaming).
- Movements from the digital twin are sent to the real robot (with active sync).
...
- Move or rotate the tool center point (TCP).
...
- **EndEffSkill**: Open/close grippers or other end effector operations (currently not available).
...
2. Test movements and skills in offline mode.
3. Establish a connection to the physical robot.
4. Execute the same skills live.
5. Monitor status and feedback live.
