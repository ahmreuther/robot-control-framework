...
function ViewportRobot({
  ...
  useEffect(() => {
    ...
    loadingManager.onLoad = () => {
      ...
      } else if (!latestRobot.homeAngles && managerJointNamesFromUrdf.length > 0) {
      ...
      finalizeFrame = window.requestAnimationFrame(() => {
        ...
        if (homeAngles && homeAngles.length === managerJointNames.length) {
          controller.getJointRuntime().startAnimationToAngles(
            robot.robotId,
            homeAngles,
            {
              durationMs: 900,
    ...
    if (isActiveIkManipulation) {
      ...
      ikJointNames.forEach((jointName, index) => {
        ...
        if (managerIndex !== undefined) {
          nextAngles[managerIndex] = result.angles[index] ?? nextAngles[managerIndex];
        }
      });
