...
function isJoint(object: unknown): boolean {
  ...
  return Boolean(
    candidate?.isURDFJoint && candidate.jointType && candidate.jointType !== "fixed",
  );
}
...
export default function DragControls({
  ...
  useEffect(() => {
    ...
    function getRevoluteDelta(
      ...
      axisWorld.copy(candidate.axis).transformDirection(candidate.matrixWorld).normalize();
    ...
    function pointerMove(event: PointerEvent) {
      ...
      if (draggingRef.current) {
        ...
        if (onUpdateJoint && dragPlaneRef.current && dragStartPointRef.current) {
          ...
          if (
            ...
            if (
              ...
              nextValue += getRevoluteDelta(joint, dragStartPointRef.current, endPoint);
    ...
    function pointerDown(event: PointerEvent) {
      ...
      const axis = new THREE.Vector3()
        ...
        .normalize();
      const pivot = new THREE.Vector3().set(0, 0, 0).applyMatrix4(candidate.matrixWorld);
